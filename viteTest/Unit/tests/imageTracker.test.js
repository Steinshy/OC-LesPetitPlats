...
import { isImageLoaded, setupImageTracking, selectRandomImages } from "@/utils/imageTracker.js";
...
describe("imageTracker", () => {
  ...
  describe("isImageLoaded", () => {
    ...
    it("should track loaded images after setupImageTracking processes them", () => {
      ...
      setupImageTracking(fragment, { jpgUrl: TEST_IMAGE_URL, webpUrl: "" });
      expect(isImageLoaded(TEST_IMAGE_URL)).toBe(true);
    });
  });

  describe("setupImageTracking", () => {
    describe("basic functionality", () => {
      it("should return early when no image URLs provided", () => {
        const fragment = createCardPictureFragment("");
        expect(() => setupImageTracking(fragment, { jpgUrl: "", webpUrl: "" })).not.toThrow();
      });
      ...
      it("should mark image as loaded when image is already loaded", () => {
        ...
        // setupImageTracking doesn't require the fragment to be in the DOM
        setupImageTracking(fragment, { jpgUrl: TEST_IMAGE_URL, webpUrl: "" });
      ...
      it("should handle missing image element gracefully", () => {
        ...
        expect(() => setupImageTracking(fragment, { jpgUrl: TEST_IMAGE_URL, webpUrl: "" })).not.toThrow();
      });
    });
    ...
    describe("webp handling", () => {
      it("should handle webp source when webp is already loaded", () => {
        ...
        // Mark webp as loaded by using setupImageTracking first
        ...
        setupImageTracking(testFragment, { jpgUrl: TEST_IMAGE_URL, webpUrl });
      ...
      it("should handle webp image load success and mark as loaded", async () => {
        ...
        // Clone fragment for setupImageTracking (since original will be emptied on append)
        const fragmentClone = fragment.cloneNode(true);
        document.body.appendChild(fragment);

        setupImageTracking(fragmentClone, { jpgUrl: TEST_IMAGE_URL, webpUrl });
        ...
        expect(webpSource).toBeTruthy();
        // Note: setupImageTracking only handles jpgUrl, not webpUrl, so isImageLoaded checks jpgUrl
        expect(isImageLoaded(TEST_IMAGE_URL)).toBe(true);
      ...
      it("should handle webp image load error and remove source", () => {
        ...
        setupImageTracking(fragment, { jpgUrl: TEST_IMAGE_URL, webpUrl: invalidWebpUrl });
      ...
      it("should handle missing webp source gracefully", () => {
        ...
        expect(() =>
          setupImageTracking(fragment, { jpgUrl: TEST_IMAGE_URL, webpUrl: TEST_WEBP_URL }),
        ).not.toThrow();
      });
      ...
      it("should handle null webpUrl", () => {
        ...
        expect(() =>
          setupImageTracking(fragment, { jpgUrl: TEST_IMAGE_URL, webpUrl: null }),
        ).not.toThrow();
      });
    });
    ...
    describe("jpeg handling", () => {
      it("should handle jpeg image error", () => {
        ...
        setupImageTracking(fragmentClone, { jpgUrl: INVALID_IMAGE_URL, webpUrl: "" });
  ...
  afterAll(() => {
    logCategorySummary("imageTracker", "Image Tracker", "All image tracker tests");
  });
});
