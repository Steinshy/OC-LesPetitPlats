...
// Re-export modern console utilities for backward compatibility
export {
  createSpinner,
  createProgressBar,
  updateProgress,
...
function logBenchmarkResult(label, value, unit = "ms", isWinner = false) {
  ...
  const color = isWinner ? chalk.green : chalk.dim;
  console.log(`  ${color(label.padEnd(20))} ${chalk.bold(formattedValue)}${unit}`);
}
...
function logComparison(winner, improvement, faster, slower) {
  ...
  console.log(
    `  ${chalk.blue("Improvement:")} ${chalk.bold(improvement.toFixed(2))}% faster than ${slower}`,
  );
  console.log(`  ${chalk.dim(`${faster} vs ${slower}`)}`);
}

function _logCategorySummaryInternal(category, stats) {
  logSection(`${category} Summary`, "📈");
  console.log(`  ${chalk.bold("Tests:")} ${stats.testCount}`);
  console.log(
    `  ${chalk.bold("Winner:")} ${chalk.green(stats.winner)} (${stats.wins}/${stats.testCount} wins)`,
  );
  console.log(`  ${chalk.bold("Avg Time:")} ${stats.avgTime.toFixed(4)}ms`);
  if (stats.improvement) {
    console.log(
      `  ${chalk.bold("Improvement:")} ${chalk.green(stats.improvement.toFixed(2))}%`,
    );
  }
}

function logMemory(label, value, unit = "MB") {
  console.log(`  ${chalk.dim(label.padEnd(20))} ${chalk.bold(value.toFixed(2))}${unit}`);
}
